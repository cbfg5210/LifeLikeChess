...
public class Rule {
	 ...
	 * 该步是否合法
	 */
	public static boolean isLegalMove(byte[][] chessBoard, Point move, byte chessColor) {

		int i, j, dirx, diry, row = move.x, col = move.y;
		if (!isLegal(row, col) || chessBoard[row][col] != Roles.EMPTY)
			return false;
	 ...
	 * 使用前务必先确认该步合法
	 */
	public static List<Point> move(byte[][] chessBoard, Point move, byte chessColor) {
		int row = move.x;
		int col = move.y;
	...
	public static List<Point> getLegalMoves(byte[][] chessBoard, byte chessColor) {
		List<Point> moves = new ArrayList<Point>();
		Point move = null;
